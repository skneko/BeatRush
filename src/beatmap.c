...
static void load_notes(Beatmap *const map, FILE *const map_file,
...
Beatmap *beatmap_load_from_file(const char *const path) {
  ...
  int argument_count =
      fscanf(map_file, "BTRM\n\n%m[^\n]\n%m[^\n]\n%m[^\n]\n\n%d\t%hu\n\n",
             &meta_info->artist, &meta_info->song_name,
             &meta_info->difficulty_name, &start_offset, &approach_time);
  if (argument_count < HEADER_ARGUMENTS) {
    printf("File does not look like a valid beatmap: %s (%d / %d)\n", path,
           argument_count, HEADER_ARGUMENTS);
    return NULL;
  }
