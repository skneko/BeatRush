 ...
 *
 * Kindly taken from
 * <https://github.com/devkitPro/3ds-examples/tree/master/audio/opus-decoding>
...
static volatile bool s_song_ongoing =
    false; // Song has started but not finished
...
unsigned long audioPlaybackPosition(void) { return songTime; }

unsigned long audioLength(void) {
  return op_pcm_total(audioFile, -1) * 1000 / SAMPLE_RATE;
}

void audioPause(void) { s_paused = true; }

void audioPlay(void) { s_paused = false; }

bool audioIsPaused(void) { return s_paused; }