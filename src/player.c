...
PlayerState player_current_state(void) { return state; }

Lane player_current_lane(void) { return current_lane; }
...
void player_draw(C2D_Sprite *player_sprite,
                 C2D_SpriteSheet player_sprite_sheet) {
  switch (state) {
  case PLAYER_STATE_HITTING: {
    int offset = HIT_ANIM_OFFSET + next_hit_animation * HIT_ANIM_LENGTH;
    int image_index = offset + anim_current_frame;
    player_sprite->image =
        C2D_SpriteSheetGetImage(player_sprite_sheet, image_index);
    break;
  }
  case PLAYER_STATE_FALLING: {
    int image_index = FALL_ANIM_OFFSET +
                      C2D_Clamp(anim_current_frame, 0, FALL_ANIM_LENGTH - 1);
    player_sprite->image =
        C2D_SpriteSheetGetImage(player_sprite_sheet, image_index);
    break;
  }
  case PLAYER_STATE_JUMPING:
  case PLAYER_STATE_FLOATING: {
    int image_index = JUMP_ANIM_OFFSET +
                      C2D_Clamp(anim_current_frame, 0, JUMP_ANIM_LENGTH - 1);
    player_sprite->image =
        C2D_SpriteSheetGetImage(player_sprite_sheet, image_index);
    break;
  }
