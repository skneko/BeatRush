...
static int w; // since all fg buildings have different widths I need a variable
              // to see where to put the next sprite
static int bird_dir;
...
static void init_sprites(void) {
  ...
  C2D_Sprite *player_sprite =
      &char_sprites[0]; // the sprite for the bg skybox, give or take you know
                        // what I mean
  ...
  C2D_Sprite *bg_sprite = &bg_sprites[0]; // the sprite for the bg skybox, give
                                          // or take you know what I mean
  ...
  for (int i = 1; i < 12; i++) {
    C2D_Sprite *bg_building_sprite =
        &bg_sprites[i]; // the sprite for the bg building
    C2D_SpriteFromSheet(bg_building_sprite, bg_sprite_sheet, 1);
    C2D_SpriteSetPos(bg_building_sprite, (i - 1) * 40,
                     BG_BUILD_MIN_HEIGHT - (rand() % BG_BUILD_MAX_HEIGHT));
  ...
  w = 0;
  for (int i = 12; i < 29; i++) {
    C2D_Sprite *fg_building_sprite =
        &bg_sprites[i]; // the sprite for the fg building
    C2D_SpriteFromSheet(fg_building_sprite, bg_sprite_sheet, 2 + (rand() % 3));
    C2D_SpriteSetPos(fg_building_sprite, w,
                     FG_BUILD_MIN_HEIGHT - (rand() % FG_BUILD_MAX_HEIGHT));
  ...
  heart_icons[IDX_HEART_FULL] =
      C2D_SpriteSheetGetImage(ui_sprite_sheet, SHEET_IDX_HEART_FULL);
  heart_icons[IDX_HEART_EMPTY] =
      C2D_SpriteSheetGetImage(ui_sprite_sheet, SHEET_IDX_HEART_EMPTY);
  ...
  C2D_SpriteSetPos(bot_bullseye_sprite, HITLINE_LEFT_MARGIN,
                   TOP_SCREEN_HEIGHT - LANE_BOTTOM_MARGIN - LANE_HEIGHT / 2);
  ...
  C2D_SpriteSetPos(top_bullseye_sprite, HITLINE_LEFT_MARGIN,
                   LANE_TOP_MARGIN + LANE_HEIGHT / 2);
  ...
  // #################################################################################################
  //_______________________________________________________________________________________________________________
  C2D_Sprite *bg_bot_sprite =
      &bot_bg_sprites[0]; // the sprite for the bg skybox, give or take you know
                          // what I mean
  C2D_SpriteFromSheet(bg_bot_sprite, bg_sprite_sheet, 8);
  C2D_SpriteSetCenter(bg_bot_sprite, .5f, .5f);
  C2D_SpriteSetPos(bg_bot_sprite, BOTTOM_SCREEN_CENTER_HOR,
                   BOTTOM_SCREEN_CENTER_VER);
  C2D_SpriteSetDepth(bg_bot_sprite, DEPTH_BG);

  // time remaining
  C2D_Sprite *player_shadow_sprite =
      &bot_bg_sprites[1]; // the sprite for the bg skybox, give or take you know
                          // what I mean
  C2D_SpriteFromSheet(player_shadow_sprite, ui_sprite_sheet, 8);
  C2D_SpriteSetCenter(player_shadow_sprite, 0, 1);
  C2D_SpriteSetPos(player_shadow_sprite, BOTTOM_SCREEN_CENTER_HOR,
                   BOTTOM_SCREEN_CENTER_VER);
...
static NoteDrawingResult draw_note(const Note *const note, int noteId) {
  long long int note_remaining_time =
      (long long int)note->position - audioPlaybackPosition();
  ...
  if (note->topLane) {
    ...
    lane_y = TOP_SCREEN_HEIGHT - LANE_BOTTOM_MARGIN - LANE_HEIGHT +
             NOTE_RADIUS + NOTE_MARGIN;
  }

  if (!note->hidden) {
    C2D_Sprite *note_sprite = &note_sprites[noteId];
    C2D_SpriteSetPos(note_sprite, floor(note_x), floor(lane_y));
    note_sprite->image = C2D_SpriteSheetGetImage(
        note_sprite_sheet, (frame % 24 < 12) ? offset : offset + 1); // animate
    C2D_DrawSprite(note_sprite);

#ifdef DEBUG_OVERLAY
    C2D_DrawLine(note_x, lane_y - NOTE_RADIUS, C2D_RED, note_x,
                 lane_y + NOTE_RADIUS, C2D_RED, 3.0f, DEPTH_DEBUG_BASE);
#endif
  }
...
static void draw_score(void) {
  ...
  prepare_text_with_font(buf, &score_label, main_font, dynamic_text_buf);
  C2D_DrawText(&score_label, C2D_WithColor | C2D_AtBaseline | C2D_AlignRight,
               390, 15, DEPTH_UI_SCORE, .35f, .35f, C2D_WHITE);
}
...
static void draw_combo(void) {
  ...
  if (combo > COMBO_DRAW_THRESHOLD) {
    ...
    prepare_text_with_font(buf, &combo_label, main_font, dynamic_text_buf);
    C2D_DrawText(&combo_label, C2D_WithColor | C2D_AtBaseline | C2D_AlignLeft,
                 TOP_SCREEN_CENTER_HOR, 25, DEPTH_UI_COMBO, .55f, .55f, color);

    prepare_text_with_font("COMBO", &combo_label, main_font, dynamic_text_buf);
    C2D_DrawText(&combo_label, C2D_WithColor | C2D_AtBaseline | C2D_AlignRight,
...
static void draw_health(void) {
  ...
  for (; i < health; i++) {
    ...
    C2D_DrawImageAt(heart_icons[IDX_HEART_FULL], x, HEART_AREA_Y,
                    DEPTH_UI_HEALTH, NULL, 1, 1);
  }
  for (; i < max_health; i++) {
    float x = HEART_AREA_X + HEART_ICON_STRIDE * i;
    C2D_DrawImageAt(heart_icons[IDX_HEART_EMPTY], x, HEART_AREA_Y,
                    DEPTH_UI_HEALTH, NULL, 1, 1);
  }
}

static void draw_attention_warnings(long long time_until_next) {
  if (flicker_is_visible(time_until_next, ATTENTION_WARN_PERIOD,
                         ATTENTION_WARN_VISIBLE)) {
    ...
    draw_sideways_triangle(left_x, top_y, ATTENTION_WARN_BASE,
                           ATTENTION_WARN_HEIGHT, 1, 1, C2D_RED, DEPTH_UI_OVER);
    draw_sideways_triangle(right_x, top_y, ATTENTION_WARN_BASE,
                           ATTENTION_WARN_HEIGHT, -1, 1, C2D_RED,
                           DEPTH_UI_OVER);
    draw_sideways_triangle(left_x, bottom_y, ATTENTION_WARN_BASE,
                           ATTENTION_WARN_HEIGHT, 1, -1, C2D_RED,
                           DEPTH_UI_OVER);
    draw_sideways_triangle(right_x, bottom_y, ATTENTION_WARN_BASE,
                           ATTENTION_WARN_HEIGHT, -1, -1, C2D_RED,
                           DEPTH_UI_OVER);
  }
}
...
static void draw_attention_cues(void) {
  if (remaining_notes_to_draw > 1) {
    long long time_until_next =
        (long long)next_note_to_draw->position - audioPlaybackPosition();
    ...
    if (in_rest) {
      ...
      prepare_text_with_font(buf, &rest_time_label, main_font,
                             dynamic_text_buf);
      C2D_DrawText(&rest_time_label,
                   C2D_WithColor | C2D_AtBaseline | C2D_AlignCenter,
                   TOP_SCREEN_CENTER_HOR, TOP_SCREEN_CENTER_VER + 5,
                   DEPTH_UI_OVER, 0.5f, 0.5f, C2D_WHITE);
    }
  }
}
...
static void draw_bg_sprites(void) {
  ...
  for (int i = 1; i < 12; i++) {
    ...
    if (bg_building_sprite->params.pos.x <= -40) {
      C2D_SpriteSetPos(
          bg_building_sprite, TOP_SCREEN_WIDTH,
          BG_BUILD_MIN_HEIGHT -
              (rand() % BG_BUILD_MAX_HEIGHT)); // 40 because that's the building
                                               // sprite's width
    }
  ...
  for (int i = 12; i < 29; i++) {
    ...
    if (fg_building_sprite->params.pos.x <= -fg_building_sprite->params.pos.w) {
      C2D_SpriteSetPos(
          fg_building_sprite, w,
          FG_BUILD_MIN_HEIGHT -
              (rand() % FG_BUILD_MAX_HEIGHT)); // 40 because that's the building
                                               // sprite's width
      w += fg_building_sprite->params.pos.w + FG_DISTANCE_BW_BUILDINGS;
    }
    if (frame % 3 == 0) {
...
static void draw_pause(void) {
  ...
  prepare_text_with_font(buf, &pause_label, main_font, dynamic_text_buf);
  C2D_DrawText(&pause_label, C2D_WithColor | C2D_AlignCenter,
               TOP_SCREEN_CENTER_HOR, TOP_SCREEN_CENTER_VER - NOTE_RADIUS - 5,
               DEPTH_PAUSE_MENU, 1.5f, 1.5f, C2D_WHITE);
}
...
static void draw_failure(void) {
  ...
  prepare_text_with_font(buf, &failure_label, main_font, dynamic_text_buf);
  C2D_DrawText(&failure_label, C2D_WithColor | C2D_AlignCenter,
               TOP_SCREEN_CENTER_HOR, TOP_SCREEN_CENTER_VER - NOTE_RADIUS - 5,
               DEPTH_PAUSE_MENU, 1.5f, 1.5f, C2D_RED);
}

static void draw_hit_popup(HitAssessment hit, C2D_Sprite *sprite, float x,
                           float y) {
...
static void draw_hit_popups(void) {
  draw_hit_popup(logic_top_hit_assessment(),
                 &hit_eval_sprites[IDX_HIT_EVAL_TOP], HIT_EVAL_TOP_X,
                 HIT_EVAL_TOP_Y);
  draw_hit_popup(logic_bottom_hit_assessment(),
                 &hit_eval_sprites[IDX_HIT_EVAL_BOTTOM], HIT_EVAL_BOTTOM_X,
                 HIT_EVAL_BOTTOM_Y);
}
...
static void draw_progress_bar(void) {
  ...
  C2D_DrawRectangle(0, TOP_SCREEN_HEIGHT - 3, DEPTH_UI_OVER,
...
static void draw_finished(void) {
  ...
  prepare_text_with_font("CLEARED!", &finished_text, main_font,
                         dynamic_text_buf);
  C2D_DrawText(&finished_text, C2D_WithColor | C2D_AtBaseline | C2D_AlignCenter,
               TOP_SCREEN_CENTER_HOR, TOP_SCREEN_CENTER_VER, DEPTH_UI_OVER,
               0.8f, 0.8f, C2D_WHITE);
  prepare_text_with_font("Press SELECT to go back to the menu",
                         &finished_text_sub, secondary_font, dynamic_text_buf);
  C2D_DrawText(&finished_text_sub,
               C2D_WithColor | C2D_AtBaseline | C2D_AlignCenter,
               TOP_SCREEN_CENTER_HOR, TOP_SCREEN_CENTER_VER + 60, DEPTH_UI_OVER,
               0.4f, 0.4f, C2D_WHITE);
}
...
static void draw_debug_song_time(void) {
  ...
  C2D_DrawText(&song_time_label, C2D_WithColor | C2D_AtBaseline, 230.0f, 220.0f,
               DEPTH_DEBUG_BASE, 0.5f, 0.5f, C2D_WHITE);
}

static void draw_debug_rulers(void) {
  // top lane
  C2D_DrawLine(0, LANE_TOP_MARGIN, C2D_GREEN, TOP_SCREEN_WIDTH, LANE_TOP_MARGIN,
               C2D_GREEN, 1.0f, DEPTH_DEBUG_RULERS);
  C2D_DrawLine(0, LANE_TOP_MARGIN + LANE_HEIGHT, C2D_GREEN, TOP_SCREEN_WIDTH,
               LANE_TOP_MARGIN + LANE_HEIGHT, C2D_GREEN, 1.0f,
               DEPTH_DEBUG_RULERS);

  // bottom lane
  C2D_DrawLine(0, TOP_SCREEN_HEIGHT - LANE_BOTTOM_MARGIN, C2D_GREEN,
               TOP_SCREEN_WIDTH, TOP_SCREEN_HEIGHT - LANE_BOTTOM_MARGIN,
               C2D_GREEN, 1.0f, DEPTH_DEBUG_RULERS);
  C2D_DrawLine(0, TOP_SCREEN_HEIGHT - LANE_BOTTOM_MARGIN - LANE_HEIGHT,
               C2D_GREEN, TOP_SCREEN_WIDTH,
               TOP_SCREEN_HEIGHT - LANE_BOTTOM_MARGIN - LANE_HEIGHT, C2D_GREEN,
               1.0f, DEPTH_DEBUG_RULERS);

  // hitline
  C2D_DrawLine(HITLINE_LEFT_MARGIN, 0.0f, C2D_WHITE, HITLINE_LEFT_MARGIN,
               TOP_SCREEN_HEIGHT, C2D_WHITE, 1.0f, DEPTH_DEBUG_RULERS);

  // hitpoints
  C2D_DrawLine(HITLINE_LEFT_MARGIN - 5.0f, LANE_TOP_MARGIN + LANE_HEIGHT / 2,
               C2D_GREEN, HITLINE_LEFT_MARGIN + 5.0f,
               LANE_TOP_MARGIN + LANE_HEIGHT / 2, C2D_GREEN, 3.0f,
               DEPTH_DEBUG_RULERS);
  C2D_DrawLine(HITLINE_LEFT_MARGIN - 5.0f,
               TOP_SCREEN_HEIGHT - LANE_BOTTOM_MARGIN - LANE_HEIGHT / 2,
               C2D_GREEN, HITLINE_LEFT_MARGIN + 5.0f,
               TOP_SCREEN_HEIGHT - LANE_BOTTOM_MARGIN - LANE_HEIGHT / 2,
               C2D_GREEN, 3.0f, DEPTH_DEBUG_RULERS);
}

static void draw_debug_keypress_hint(bool top_lane) {
  float lane_y =
      (top_lane ? LANE_TOP_MARGIN
                : TOP_SCREEN_HEIGHT - LANE_BOTTOM_MARGIN - LANE_HEIGHT) +
      LANE_HEIGHT / 2;

  C2D_DrawCircleSolid(HITLINE_LEFT_MARGIN, lane_y, DEPTH_DEBUG_BASE,
                      NOTE_RADIUS / 2, C2D_ORANGE);
}
...
static void draw_debug_invincibility_hint(void) {
  if (logic_is_invencible()) {
    ...
    prepare_text_with_font(message, &invincibility_hint_label, NULL,
                           dynamic_text_buf);
    C2D_DrawText(&invincibility_hint_label,
                 C2D_WithColor | C2D_AtBaseline | C2D_AlignCenter, 50, 230,
                 DEPTH_DEBUG_BASE, 0.5f, 0.5f, C2D_BLUE);
  }
}
...
static void draw_debug_auto_hint(void) {
  ...
  C2D_DrawText(&auto_hint_label,
               C2D_WithColor | C2D_AtBaseline | C2D_AlignCenter,
               TOP_SCREEN_CENTER_HOR, 50, DEPTH_DEBUG_BASE, 0.6, 0.6, C2D_RED);
}
#endif
...
static void draw_hit_counters(void) {
  // perfect
  C2D_Text perfect_label;
  char buf[10];
  snprintf(buf, sizeof(buf), "%u", logic_hit_count_perfect());
  prepare_text_with_font(buf, &perfect_label, main_font, dynamic_text_buf);
  C2D_DrawText(&perfect_label, C2D_WithColor | C2D_AtBaseline | C2D_AlignLeft,
  ...
  prepare_text_with_font(buf, &good_label, main_font, dynamic_text_buf);
  C2D_DrawText(&good_label, C2D_WithColor | C2D_AtBaseline | C2D_AlignLeft, 77,
               21, DEPTH_UI_SCORE, .25f, .25f, C2D_Color32(218, 191, 255, 255));

  // ok
  C2D_Text ok_label;
  snprintf(buf, sizeof(buf), "%u", logic_hit_count_ok());
  prepare_text_with_font(buf, &ok_label, main_font, dynamic_text_buf);
  C2D_DrawText(&ok_label, C2D_WithColor | C2D_AtBaseline | C2D_AlignLeft, 77,
               31, DEPTH_UI_SCORE, .25f, .25f, C2D_Color32(218, 191, 255, 255));

  // miss
  C2D_Text miss_label;
  snprintf(buf, sizeof(buf), "%u", logic_hit_count_miss());
  prepare_text_with_font(buf, &miss_label, main_font, dynamic_text_buf);
  C2D_DrawText(&miss_label, C2D_WithColor | C2D_AtBaseline | C2D_AlignLeft, 77,
               41, DEPTH_UI_SCORE, .25f, .25f, C2D_Color32(218, 191, 255, 255));
}
...
static void draw_song_name(void) {
  ...
  C2D_DrawText(&songLabel, C2D_WithColor | C2D_AtBaseline | C2D_AlignLeft, 50,
               217, DEPTH_UI_SCORE, .3f, .3f, C2D_Color32(218, 191, 255, 255));
  ...
  C2D_DrawText(&artistLabel, C2D_WithColor | C2D_AtBaseline | C2D_AlignLeft, 60,
               228, DEPTH_UI_SCORE, .3f, .3f, C2D_Color32(218, 191, 255, 255));
}

static void draw_time_remaining(void) {
  float progress = (float)audioPlaybackPosition() / audioLength();
  C2D_Sprite *player_shadow_sprite = &bot_bg_sprites[1];
  C2D_SpriteSetPos(player_shadow_sprite, (BOTTOM_SCREEN_WIDTH - 37) * progress,
                   210);
  player_shadow_sprite->image = C2D_SpriteSheetGetImage(
      ui_sprite_sheet, (frame % 30 == 0) ? 8 + ((++shadow_anim_frame) % 10)
                                         : 8 + ((shadow_anim_frame) % 10));
