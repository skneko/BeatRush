...
const char *options[] = {"[Easy  ] K/DA - POP/STARS",
                         "[Normal] Haloweak - Bass Telekinesis",
                         "[Hard  ] Memtrix - All You Are",
                         "[Insane] Panda Eyes & Terminite - Highscore", "Quit"};
...
void draw_unselected_option(int i, float x, float y) {
  ...
  C2D_DrawText(&optionText, C2D_WithColor, x, y, MENU_DEPTH, 0.5f, 0.5f,
               C2D_WHITE);
}
...
void draw_options(void) {
  ...
  C2D_TextOptimize(&optionText);
  C2D_DrawText(&optionText, C2D_WithColor,
               MENU_MARGIN_LEFT - MENU_SELECTED_INDENT, TOP_SCREEN_CENTER_VER,
               MENU_DEPTH, 0.5f, 0.5f, C2D_WHITE);

  draw_sideways_triangle(MENU_MARGIN_LEFT - MENU_SELECTED_INDENT - 20,
                         TOP_SCREEN_CENTER_VER + 2, 8, 10, 1, 1, C2D_RED,
                         MENU_DEPTH);
}
...
Beatmap *select_beatmap(void) {
  ...
  case 1: {
    ...
    return beatmap_load_from_file(
        "romfs:/beatmaps/bassTelekinesis/beatmap.btrm");
  }
...
void menu_end(void) { C2D_TextBufDelete(dynamic_text_buf); }